...
function loadFcpData(contentFolder, dataFolder, stage) {
  const data = loadRFCData(contentFolder, dataFolder);

  const result = [];

  for (let fileContents of data) {
    if (fileContents.currentStage === stage && fileContents.fcp) { //ToDo make sure we're getting the correct stages here, because we want the Fcp from exploring to accepted so not accepted
      result.push(fileContents);
    }
  }
...
export default function tocBuilder(contentFolder, dataFolder) {
  return {
    ...
    load(id) {
      ...
      if (id.startsWith('rfcs-app-toc-builder:fcp-')) {
        const stage = id
          .replace(/^rfcs-app-toc-builder:fcp-/, '')
          .replace(/\.json$/, '');
        this.addWatchFile(contentFolder);
        this.addWatchFile(dataFolder);
        return loadFcpData(contentFolder, dataFolder, stage);
      }
