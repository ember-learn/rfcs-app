...
function loadRFCData(contentFolder, dataFolder) {
  ...
  for (let file of files) {
    ...
    for(let copyItem of ['start-date', 'release-date', 'release-versions', 'teams', 'prs', 'project-link', 'meta', '__content']) {
  ...
  return {
    ...
    load(id) {
      ...
      if (id.startsWith('rfcs-app-toc-builder:stage-')) {
        const stage = id.replace(/^rfcs-app-toc-builder:stage-/, '').replace(/\.json$/, '');
        this.addWatchFile(contentFolder);
        this.addWatchFile(dataFolder);
